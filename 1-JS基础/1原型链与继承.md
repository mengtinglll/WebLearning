...
- [继承（ES5）](#继承es5)
  ...
  - [2原型链实现继承](#2原型链实现继承)
  - [3组合继承方式](#3组合继承方式)
  - [组合继承方式 优化1：](#组合继承方式-优化1)
  - [组合继承方式 优化2 - 添加中间对象【最通用版本】：](#组合继承方式-优化2---添加中间对象最通用版本)
  - [封装一个原生的继承方法](#封装一个原生的继承方法)
...
```
### 构造函数constructor
- 在ES6之前，没有class，直接通过构造函数来创建实例
- 构造函数就是一个普通的函数，创建方式和普通函数没有区别，但需要使用new关键字来调用。

### 原型
- JavaScript 的所有对象中都包含了一个 `__proto__` 内部属性，这个属性所对应的就是该对象的原型
 - JavaScript 的函数对象，除了原型 `__proto__` 之外，还预置了 prototype 属性！！！
 - 函数的prototype属性被所有实例共享，它是一个对象数据类型的值，称原型对象。
...
- 对象和对象之间也有关系，并不是孤立存在的。对象之间的继承关系，在JavaScript中是通过proto指向父类对象，直到指向Object对象为止，这样就形成了一个原型指向的链条，专业术语称之为原型链。
  
- 任何一个实例对象通过原型链可以找到它对应的原型对象，原型对象上面的实例和方法都是实例所共享的。
  
- 一个对象在查找以一个方法或属性时，他会先在自己的对象上去找，找不到时，他会沿着原型链依次向上查找。

- ES6子类的__proto__属性，表示构造函数的继承，总是指向父类。
- ES6子类prototype属性的__proto__属性，表示方法的继承，总是指向父类的prototype属性。
...
### [ES5/ES6 的继承除了写法以外还有什么区别？](https://github.com/Advanced-Frontend/Daily-Interview-Question/issues/20)
 ...
 - 必须使用 new 调用 class。
 - class 内部无法重写类名。
 - es5实质是先创造子类的实例对象this，再将父类方法给到this。先将父类实例对象的属性和方法，加到this上面（所以必须先调用super方法），然后再用子类的构造函数修改this。
