- [JS 变量类型](#js-变量类型)
- [JS中使用typeof能得到哪些类型？](#js中使用typeof能得到哪些类型)
- [instanceof 能正确判断对象的原理是什么？](#instanceof-能正确判断对象的原理是什么)
- [实现一个类型判断函数](#实现一个类型判断函数)
- [转Boolean](#转boolean)
- [对象转基本类型](#对象转基本类型)
- [隐式类型转换（自动类型转换）](#隐式类型转换自动类型转换)
  - [递增/递减、正负符号](#递增递减正负符号)
  - [加法运算符](#加法运算符)
  - [乘除、减号、取模](#乘除减号取模)
- [`100 +` 问题](#100--问题)
- ["a common string"为什么会有length属性](#a-common-string为什么会有length属性)
- [== 操作符](#-操作符)
- [=== 操作符](#-操作符-1)
- [{} 等于true还是false](#-等于true还是false)
- [1 与 Number(1)有什么区别](#1-与-number1有什么区别)
- [console.log(!!(new Boolean(false))输出什么 [易混淆]](#consolelognew-booleanfalse输出什么-易混淆)
- [如何判断一个数据是不是Array](#如何判断一个数据是不是array)
- [Object.prototype.toString](#objectprototypetostring)
- [obj.toString() 和Object.prototype.toString.call(obj)](#objtostring-和objectprototypetostringcallobj)
- [输出以下代码运行结果](#输出以下代码运行结果)
### JS 变量类型
...
### 转Boolean
以下都为假值，其他所有值都转为 true，包括所有`对象（空对象，空数组也转为真）`。
...
- null，转为0
- undefined，转为NaN
- 字符串：仅数字，转为数字；空，转0；其余，转NaN
- 对象：调用valueOf()，用以上规则；若结果为NaN，则调用toString()
...
#### 加法运算符
- 若有NaN，结果为NaN
- 若有字符串：两个字符串，拼接；一个字符串，将另一个转为字符串toString()，拼接
- 布尔值，转1和0
```javascript
1 + '1' // '11'
true + true // 2
