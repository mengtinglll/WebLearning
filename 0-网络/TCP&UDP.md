- [TCP](#tcp)
    ...
    - [为什么不用两次握手？](#为什么不用两次握手)
    - [四次挥手](#四次挥手)
    - [为什么 A 要进入 TIME-WAIT 状态，等待 2MSL 时间后才进入 CLOSED 状态？](#为什么-a-要进入-time-wait-状态等待-2msl-时间后才进入-closed-状态)
    - [为什么建立连接是三次握手，关闭连接确是四次挥手呢？](#为什么建立连接是三次握手关闭连接确是四次挥手呢)
  - [ARQ （超时重传）协议](#arq-超时重传协议)
    - [停止等待 ARQ](#停止等待-arq)
    - [连续 ARQ](#连续-arq)
      - [累计确认](#累计确认)
      - [滑动窗口](#滑动窗口)
...
对于 TCP 头部来说，以下几个字段是很重要的:
 ...
 - `标识符`
   ...
   - SYN=1：当SYN=1，ACK=0时，表示当前报文段是一个连接请求报文。<br>当SYN=1，ACK=1时，表示当前报文段是一个同意建立连接的应答报文。
